...
const buildScreenContent = (goal: GoalType) => {
  if (goal === "lose_fat") {
    return {
      ...
      subtitle:
        "Choose how aggressively you want calories to sit below maintenance.",
  ...
  return {
    ...
    icon: (
      <TrendUpIcon size={22} color={appColors.statusSuccess} weight="fill" />
    ),
    notes: [
      ...
      <View style={styles.optionsWrap}>
        {content.options.map((option) => (
          <OnboardingButton
            ...
            subtitle={[
              `About ${option.approxWeeklyRateKg} kg/week · ${formatSignedCalories(option.dailyCalorieDelta)}`,
